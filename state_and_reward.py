...
#popravil, da zdej pogleda ce lahko karkoli igra
#ce lahko vrne true in kart ce ne vrne false
def strong_open(hand):
	...
	if len(l) > 0:
		l = list(sorted(l, key=lambda x: x[1]))[-1]
...
def weak_open(hand):
	...
	#maxherc = int(str(herc[-1])[-1])
	if len(herc) > 0:
		maxherc = int(str(herc[0])[-1])
		l.append((0, maxherc))
	
	#maxkaro = int(str(karo[-1])[-1])
	if len(karo) > 0:
		maxkaro = int(str(karo[0])[-1])
		l.append((1, maxkaro))
	
	#maxpik = int(str(pik[-1])[-1])
	if len(pik) > 0:
		maxpik = int(str(pik[0])[-1])
		l.append((2, maxpik))
	
	#maxkriz = int(str(kriz[-1])[-1])
	if len(kriz) > 0:
		maxkriz = int(str(kriz[0])[-1])
		l.append((3, maxkriz))
	...
	if len(l) > 0:
		#print("V IFU")
		l = list(sorted(l, key=lambda x: x[1]))[0]
		if l[0] == 0:
			return True, herc[0]
		elif l[0] == 1:
			return True, karo[0]
...
def strong_tarock(hand):
	...
	if len(high_taroki) > 0:
		return True, high_taroki[0]
	else:
		return False, None
...
def take_high(legal_moves, stack):
	...
	if len(taking_moves) == 1:
		...
		return True, taking_moves[1]
	elif len(taking_moves) > 2:
		return True, taking_moves[len(taking_moves)//2]
	else:
		return False, None
...
def partner_open(state_duo_first, legal_moves): #legal moves here should just be hand because this is the opening action
	if state_duo_first[-1] != 1:
		#to pomeni da moj parnter ni zadnji (player2)
...
def action_solo_first(hand, action):
	...
	#treba pohendlat, da enkrat ene akcije lahko drugic ne
	if action == 0:
		succ, card = strong_open(hand)
		if not succ:
			succ2, card2 = weak_open(hand)
			if not succ2:
				return hand[0]
			else:
				return card2
		...
		succ, card = strong_tarock(hand)
		if not succ:
			succ2, card2 = weak_tarock(hand)
			if not succ2:
				return hand[0]
			else:
				return card2
...
def action_solo_second(legal_moves, action, stack):
	...
	if action == 0:
		succ, card = take_low(legal_moves, stack)
		if not succ:
			succ2, card2 = take_high(legal_moves, stack)
			if not succ2:
		...
		succ, card = take_allin(legal_moves, stack)
		if not succ:
			return legal_moves[0]
		else:
			return card
...
def reward_solo_first(tarock_instance, stack, stack_order, hand, action):
	...
	if action == 1 and winner != me:
		...
		#print(legal_moves, "B")
		#print(hand, "C")
		#kratki stik poskrbi da ne probas pogledat kaj bi lahko pobral ce je bla itak zadnja karta
		if len(hand) > 0 and len(lahko_poberes(stack[1:], legal_moves)) > 0:
			reward += -1*stack_value
...
def reward_solo_third(tarock_instance, stack, stack_order, hand, action):
	...
	if isTarock(my_card) and action == 1 and len(poberes) > 0:
		if min(poberes) < my_card:
			reward += -2

	if isTarock(my_card) and action == 2 and len(poberes) > 0:
		if min(poberes) < my_card:
			reward += -2
...
def reward_duo_third(tarock_instance, stack, stack_order, hand, action, kolega):
	...
	if isTarock(my_card) and action == 1 and winner in us and len(poberes) > 0:
		if min(poberes) < my_card:
			reward += -2

	if isTarock(my_card) and action == 2 and winner in us and len(poberes) > 0:
		if min(poberes) < my_card:
			reward += -2
